...
namespace XLua.CSObjectWrap
{
    ...
    public class ListenerTypeWrap
    {
		public static void __Register(RealStatePtr L)
        {
		    ObjectTranslator translator = ObjectTranslatorPool.Instance.Find(L);
		    Utils.BeginObjectRegister(typeof(ListenerType), L, translator, 0, 0, 0, 0);
			Utils.EndObjectRegister(typeof(ListenerType), L, translator, null, null, null, null, null);
			
			Utils.BeginClassRegister(typeof(ListenerType), L, null, 3, 0, 0);
            
            Utils.RegisterObject(L, translator, Utils.CLS_IDX, "Normal", ListenerType.Normal);
            
            Utils.RegisterObject(L, translator, Utils.CLS_IDX, "Extend", ListenerType.Extend);
            
			Utils.RegisterFunc(L, Utils.CLS_IDX, "__CastFrom", __CastFrom);
            
            Utils.EndClassRegister(typeof(ListenerType), L, translator);
        }
        ...
        static int __CastFrom(RealStatePtr L)
		{
            ...
            if (lua_type == LuaTypes.LUA_TNUMBER)
            {
                translator.PushListenerType(L, (ListenerType)LuaAPI.xlua_tointeger(L, 1));
            }
            ...
            else if(lua_type == LuaTypes.LUA_TSTRING)
            {
                ...
                {
                    translator.PushListenerType(L, ListenerType.Normal);
                }
				else if (LuaAPI.xlua_is_eq_str(L, 1, "Extend"))
                {
                    translator.PushListenerType(L, ListenerType.Extend);
                }
				else
                {
                    return LuaAPI.luaL_error(L, "invalid string for ListenerType!");
                }
            }
			
            else
            {
                return LuaAPI.luaL_error(L, "invalid lua type for ListenerType! Expect number or string, got + " + lua_type);
            }

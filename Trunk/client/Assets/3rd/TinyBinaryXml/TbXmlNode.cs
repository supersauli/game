...
namespace TinyBinaryXml
{
	public class TbXmlNode
	{
        ...
        public double GetDoubleValue(string name, double defaultValue = 0.0f)
        {
            object value = GetValue(ref name);
            if (value is double)
            {
        ...
        public long GetLongValue(string name, long defaultValue = 0)
        {
            object value = GetValue(ref name);
            if (value is double)
            {
        ...
        public List<string> GetKeyList()
        {
            ...
            TbXmlNodeTemplate nodeTemplate = tbXml.nodeTemplates[templateId];
            foreach (var v in nodeTemplate.attributeNameIndexMapping)
            {
                list.Add(v.Key);
            }
            return list;
